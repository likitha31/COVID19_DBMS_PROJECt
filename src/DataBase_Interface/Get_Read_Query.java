		...
		//To know the total cases in a hospital
			...
			query =" SELECT person.First_Name,person.Last_Name  ,person.Age, T.Doctor_ID, T.Patient_ID, T.Date_Admit , T.Date_Discharge , T.Testing_Status"+
					 " from  Person inner join "+
					  ...
					  "   where Doctor.Hospital_id  =  P.Hospital_id) as T "+
			...
			query =" SELECT Doctor.NAME AS NAME,Doctor.DOCTOR_ID AS DOCTOR_ID ,Doctor.QUALIFICATION as QUALIFICATION, Count(Patient.patient_id) as TOTAL_PATIENTS, "+
				...
				"	      where Doctor.Hospital_id  =  Patient.Hospital_id "+
		...
		//	values[0] = (String) obj.get("Hospital_ID");
			...
			query =" SELECT person.First_Name,person.Last_Name  ,person.Age, T.Patient_ID, T.Date_Admit , T.Date_Discharge , T.Testing_Status"+
					 " from  Person inner join "+
					   ...
					   "  on Doctor.Doctor_ID = P.Doctor_ID "+
					  "   where Doctor.DOCTOR_id  =  P.Doctor_id) as T "+
					 " on T.aadhar_number = person.aadhar_number "+
					"  ORDER BY T.DATE_DISCHARGE DESC ;";
			...
			query ="SELECT  Hospital.Name as Name, City, State,Count(Patient_ID)  as Patient, Count(Distinct Doctor_ID)  as Doctor from Hospital INNER JOIN (Select pt.Aadhar_Number, Doctor.Hospital_ID , Doctor.Doctor_ID , pt.Patient_ID from Doctor INNER JOIN Patient pt on Doctor.Doctor_ID = pt.Doctor_ID)as der On der.Hospital_ID = Hospital.Hospital_id "+
			"  where Hospital.city = (Select pr.city from person pr where pr.`Aadhar_Number` = der.`Aadhar_Number`) "+
					" and Hospital.state =(Select pr.state from person pr where pr.`Aadhar_Number` = der.`Aadhar_Number`) "
...
//			features[8]="PinCode";
			...
			query = "Select * from teams order by state";
		...
		else if (type.equals("Date_Analysis")) {
			query ="SELECT date(Date_Admit),COUNT(*) FROM patient GROUP BY date(Date_Admit);";
			values = new String[0];
			//values[0] = (String) obj.get("date");
			features = new String[2];
			features[0] = "date(Date_Admit)";
			features[1] = "COUNT(*)";
